...
`code.py` is now a compatibility entry point. The runtime is split by
responsibility under `coding_assistant/`:

```text
code.py                         CLI compatibility entry point
coding_assistant/
  config.py                     environment, model client, console I/O
  memory.py                     optional adapter for the s09 memory runtime
  tasks.py                      task graph and task-bound worktrees
  skills.py                     skill discovery and system prompt assembly
  tools.py                      shell, file, glob, and todo tools
  hooks.py                      lifecycle hooks and permission policy
  subagents.py                  one-shot isolated subagents
  teams.py                      persistent teammates, mailboxes, plan protocol
  compaction.py                 context budgeting, transcripts, recovery
  background.py                 background shell jobs and notifications
  cron.py                       durable scheduling and delivery acknowledgement
  mcp.py                        MCP clients, mock servers, host policy metadata
  registry.py                   tool schemas, handlers, dynamic tool assembly
  agent.py                      integrated model loop and event wakeups
  cli.py                        interactive command-line host
```

The dependency direction is intentionally centered on `agent.py`: capability
modules own their state and handlers, `registry.py` exposes them to the model,
and `agent.py` coordinates them. Add a new local tool to its capability module
and register its schema/handler in `registry.py`; add a new event source beside
`background.py` or `cron.py` and inject it from `agent.py`.

The memory integration remains compatible with the course's sibling
`s09_memory/code.py`. Set `MEMORY_RUNTIME_PATH` to use a different location. If
no memory runtime is available, the assistant starts with memory disabled
